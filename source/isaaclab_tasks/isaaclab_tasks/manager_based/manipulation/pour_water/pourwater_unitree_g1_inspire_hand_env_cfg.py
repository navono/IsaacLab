...
# Locate the repo root that contains the shared supcon assets to avoid constructing an invalid
# path relative to the manager_based package.
PROJECT_ROOT = next(
    (p for p in CURRENT_DIR.parents if (p / "assets").exists()), CURRENT_DIR
)
...
##
@configclass
class PourWaterSceneCfg(InteractiveSceneCfg):
    ...
    cup = RigidObjectCfg(
        ...
        # Yaw about world Z ≈ -133.149 deg -> normalized quaternion (w, x, y, z)
        init_state=RigidObjectCfg.InitialStateCfg(pos=[0.02821, 0.47534, 1.06226], rot=[1, 0.0, 0.0, 0.0]),
        spawn=UsdFileCfg(
            usd_path=f"{ASSETS_DIR}/cup.usdc",
...
class SubtaskTermsCfg(ObsGroup):
    """Observations for subtask termination signals."""

    enable_corruption: bool = False
    grasp_bottle = ObsTerm(func=mdp.grasp_bottle)
    move = ObsTerm(func=mdp.bottle_above_cup_right)
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        cup_rot = ObsTerm(func=base_mdp.root_quat_w, params={"asset_cfg": SceneEntityCfg("cup")})
...
class TerminationsCfg:
    ...
    bottle_dropping = DoneTerm(
        func=mdp.root_height_below_minimum, params={"minimum_height": 0.5, "asset_cfg": SceneEntityCfg("bottle")}
    )

    cup_dropping = DoneTerm(
        func=mdp.root_height_below_minimum, params={"minimum_height": 0.5, "asset_cfg": SceneEntityCfg("cup")}
    )

    success = DoneTerm(func=mdp.task_done_bottle_pour, params={})
...
@configclass
class PourWaterG1InspireFTPEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the GR1T2 environment."""

    # Scene settings
    scene: PourWaterSceneCfg = PourWaterSceneCfg(num_envs=1, env_spacing=2.5, replicate_physics=True)
    # Basic settings
